...
class AntTransformer(nn.Module):
    ...
    def __init__(self, config: AntConfig):
        ...
        # ── 分类头 ──────────────────────────────────────────────
        out_dim = 3 if getattr(config, "use_quantile_head", False) else config.num_classes
        self.classifier = nn.Sequential(
            nn.Linear(config.d_model, config.d_model // 2),
            nn.GELU(),
            nn.Dropout(config.dropout),
            nn.Linear(config.d_model // 2, out_dim),
        )
    ...
    def forward(
        ...
        # ④ 分类
        raw_logits = self.classifier(cls_repr)  # [B, out_dim]

        if getattr(self.config, "use_quantile_head", False):
            ...
            q50 = base
            q10 = base - F.softplus(a)
            q90 = base + F.softplus(b)

            logits = torch.stack([q10, q50, q90], dim=-1) # [B, 3]
        else:
            logits = raw_logits
