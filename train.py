...
from model.losses import PearsonCorrLoss, WeightedPinballLoss
...
def get_predictions(model, loader, device, features, target):
    ...
    for batch in tqdm(loader, desc="  predicting", leave=False):
        ...
        if len(logits.shape) > 1 and logits.shape[-1] == 3: # Quantile shape
            q10 = np_logits[:, 0]
            q50 = np_logits[:, 1]
            q90 = np_logits[:, 2]
            
            # Application of Asymmetric Snail Shell
            from core.asymmetric_mechanism import AsymmetricSnailMechanism
            snail = AsymmetricSnailMechanism()
            # q50 serves as both point_pred and anchor since we use e2e Transformer output
            corrected_pred, _, _, _ = snail.apply(q50, q50, q10, q90, beta=1.0)
            
            all_preds.append({
                "pred_raw": q50.flatten(),
                "pred": corrected_pred.flatten(),
                "q10": q10.flatten(),
                "q90": q90.flatten()
            })
        else:
            flat_pred = np_logits.squeeze(-1).flatten()
            all_preds.append({
                "pred_raw": flat_pred,
                "pred": flat_pred
            })
    ...
    # 展开预测的 dict list
    pred_raw_concat = np.concatenate([p["pred_raw"] for p in all_preds])
    pred_concat     = np.concatenate([p["pred"] for p in all_preds])
    
    df_out = pd.DataFrame({
        "ticker": all_tickers,
        "date": all_dates,
        "target": np.concatenate(all_targets),
        "pred_raw": pred_raw_concat,
        "pred": pred_concat,
    })
    
    # 增加 q10, q90 输出（如果存在）
    if "q10" in all_preds[0]:
        df_out["q10"] = np.concatenate([p["q10"] for p in all_preds])
        df_out["q90"] = np.concatenate([p["q90"] for p in all_preds])
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="mse",
        choices=["mse", "ic", "pinball"],
        help="损失函数类型: mse 或 ic (Pearson Correlation), 或 pinball (对于 quantiles)",
    )
    ...
    elif args.loss_type == "pinball":
        criterion = WeightedPinballLoss()
        logger.info("使用 Weighted Pinball Loss 用于分位数回归")
    else:
